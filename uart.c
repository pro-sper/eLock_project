...
void uart0_init()
{
	...
	EUSCI_A0->CTLW0 |= BIT0;
	EUSCI_A0->CTLW0 |= 0x80;			//Q: Why '|=' ? This sets the mode to synchronous. 
																//	 I thought the UART was supposed to be async?
...
BYTE uart0_getchar()
{
	...
	// Wait for data
	// IFG register
	while((EUSCI_A0->IFG & BIT0) == 0){};		//BIT0 of the Interrupt flag register us the receive interrupt flag	
...
void uart0_putchar(char ch)
{
	...
	// IFG register
	// UCTXIFG is set when new data can be written into UCAxTXBUF
	while((EUSCI_A0->IFG & BIT1) == 0){}; //BIT1 of the Interrupt flag register us the transmit interrupt flag	
